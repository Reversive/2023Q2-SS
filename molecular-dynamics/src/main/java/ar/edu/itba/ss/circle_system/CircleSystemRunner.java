...
public class CircleSystemRunner {
    ...
    private static final double minUi = 9, maxUi = 12;
...
//        int[] Ns = {30};
    ...
    private static boolean isColliding(double position, List<Particle> particles) {
        for(Particle p : particles) {
            double distance = Math.min(L - Math.abs(p.getPosition() - position) , Math.abs(position - p.getPosition()));
            if(distance < 2*r)
                return true;
        }
        return false;
    ...
    private static List<Particle> gearMethod(List<Particle> particles, double dt) {
        ...
        for(Particle p1 : particles) {
            ...
            double[] rp = predictByTaylor(particle.getX0()%L, p1.getX1(), p1.getX2(), p1.getX3(), p1.getX4(), p1.getX5(), particle.getPosition(), dt);
            particle.setX0(rp[0]%L);
            particle.setX1(rp[1]);
            ...
            particle.setX0((rp[0] + coef[0] * deltaR2) % L);
    ...
    private static double collisionForce(Particle p1, Particle p2) {
        return K * (Math.abs(p1.getX0() - p2.getX0()) - 2*p1.getR()) * Math.signum(p1.getX0() - p2.getX0());
    }

    private static double propulsionForce(Particle p) {
        return (p.getUi() - p.getX1());
    }
    ...
    private static double[] predictByTaylor(double r, double r1, double r2, double r3, double r4, double r5, double position, double dt) {
        ...
        double r4p = r4 + r5 * dt;
        return new double[]{rp % L, r1p, r2p, r3p, r4p, r5, rpPosition};
    }
}
